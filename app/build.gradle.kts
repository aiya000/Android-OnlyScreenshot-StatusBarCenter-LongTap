...
android {
    namespace = "io.github.aiya000.onlyscreenshot"
    compileSdk = 35

    defaultConfig {
        applicationId = "io.github.aiya000.onlyscreenshot"

        // AccessibilityService.takeScreenshot() and its canTakeScreenshot attribute are
        // API 30, and so is WindowManager.getCurrentWindowMetrics(). The app is nothing
    ...
    signingConfigs {
        if (releaseStoreFile != null) {
            create("release") {
                ...
                // v1 (JAR signing) is only needed below API 24 and minSdk is 30. v3 is
    ...
    buildTypes {
        debug {
            ...
            // (src/debug/res) and its label says debug. The two accessibility services
            // then show up separately in the system settings as well.
