...
description: Install the signed release APK of this OnlyScreenshot app on the connected device with adb. Use when the user asks to install or deploy the production / release build; build it first with the `release-build` skill if needed.
---
...
4. Only where that fails with `INSTALL_FAILED_UPDATE_INCOMPATIBLE`, uninstall the installed copy and
   install again. Do not uninstall pre-emptively -- it also clears the app's data:
    ...
    ```bash
    adb uninstall io.github.aiya000.onlyscreenshot
...
- Debug and release have different application ids -- `io.github.aiya000.onlyscreenshot.debug` and
  `io.github.aiya000.onlyscreenshot` (`applicationIdSuffix` in `app/build.gradle.kts`) -- so both are
...
- `INSTALL_FAILED_UPDATE_INCOMPATIBLE` means the installed copy was signed with a different key -- an
  ...
  of step 4, which also clears the saved settings
