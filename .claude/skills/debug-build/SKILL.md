---
name: debug-build
description: Build the debug APK of this OnlyScreenshot app with gradle. Use when the user asks for a debug build, or before installing a debug build with the `debug-install` skill.
---
...
- The debug variant's application id is `io.github.aiya000.onlyscreenshot.debug` (`applicationIdSuffix`),
  so it installs side by side with the release build. `src/debug/res` overrides the launcher icon background
  with orange and the label with `OnlyScreenshot debug`, which is how the two are told apart -- **including in
  the accessibility settings**, where both services would otherwise be called the same thing
